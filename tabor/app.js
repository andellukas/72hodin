...
(function () {
  ...
  function renderContacts(txt){
    const lines = String(txt ?? "").split("\n");
    const out = [];
    for(const raw of lines){
      const line = raw; // keep spaces
      // heading: starts with emoji/symbol + optional spaces + digit + ')'
      if (/^[^\w\s]\s*\d+\)\s+/.test(line)) {
        out.push('<div class="cLine"><span class="cH">'+esc2(line)+'</span></div>');
      } else if (line.trim() === "────────────────────────") {
        out.push('<div class="cSep" aria-hidden="true"></div>');
      } else if (line.trim() === "") {
        out.push('<div class="cGap"></div>');
      } else {
        out.push('<div class="cLine">'+esc2(line)+'</div>');
      }
    }
    body.innerHTML = out.join("");
  }

  renderContacts(CONTACTS_TEXT);
